...
@section('title', 'Posters - Admin Dashboard')
@section('page-title', 'Posters')
...
<div class="space-y-6">
    ...
    <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border border-gray-200">
        ...
        <div class="p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 via-white to-gray-50">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <h3 class="text-xl font-bold text-[#fe5000]">Posters Directory</h3>
                        <p class="text-sm text-[#fe5000] mt-1">Manage promotional and informational posters</p>
                    </div>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
                    <div class="text-sm text-gray-600 bg-gray-100 px-4 py-2 rounded-xl">
                        <span class="text-[#fe5000] font-semibold">{{ $posters->firstItem() ?? 0 }}-{{ $posters->lastItem() ?? 0 }}</span> of <span class="text-[#fe5000] font-semibold">{{ $posters->total() }}</span> posters
                    </div>
        ...
        <div class="p-6 border-b border-gray-200 bg-gray-50">
            <form method="GET" action="{{ route('admin.posters.index') }}" class="space-y-4">
        ...
        <div class="overflow-x-auto">
            @if($posters->count() > 0)
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Poster</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($posters as $poster)
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-12 w-12">
                                            @if($poster->image_path)
                                                <img class="h-12 w-12 rounded-xl object-cover border border-gray-200" src="{{ asset('storage/' . $poster->image_path) }}" alt="{{ $poster->title }}">
                                            @else
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-semibold text-gray-900">
                                                <a href="{{ route('admin.posters.show', $poster) }}" class="hover:text-[#fe5000] transition-colors duration-200">
                                                    {{ Str::limit($poster->title, 40) }}
                                                </a>
                                            </div>
                                            <div class="text-sm text-gray-500">{{ Str::limit($poster->description, 60) }}</div>
                                            @if($poster->category)
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 mt-1">
                                                    {{ ucfirst($poster->category) }}
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($poster->campaign)
                                        <div class="text-sm font-semibold text-gray-900">{{ Str::limit($poster->campaign->title, 30) }}</div>
                                        <div class="text-sm text-gray-500">Campaign</div>
                                    @else
                                        <div class="text-sm text-gray-500">No campaign</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-900">{{ ucfirst($poster->type ?? 'General') }}</div>
                                    <div class="text-sm text-gray-500">{{ $poster->format ?? 'Digital' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold shadow-sm
                                        @if($poster->status === 'published') bg-gradient-to-r from-green-100 to-green-200 text-green-800 border border-green-200
                                        @elseif($poster->status === 'draft') bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 border border-gray-200
                                        @elseif($poster->status === 'archived') bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border border-blue-200
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div>{{ $poster->created_at->format('M d, Y') }}</div>
                                    <div class="text-xs text-gray-400">{{ $poster->created_at->diffForHumans() }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.posters.show', $poster) }}" 
                                        ...
                                        <a href="{{ route('admin.posters.edit', $poster) }}" 
                                           class="inline-flex items-center px-3 py-2 border border-gray-300 rounded-lg shadow-sm text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#fe5000] transition-all duration-200">
                                            <svg class="-ml-1 mr-1 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                ...
                <div class="text-center py-12">
                    <div class="mx-auto h-24 w-24 bg-gradient-to-br from-[#fe5000]/10 to-orange-100 rounded-2xl flex items-center justify-center mb-6">
                        <svg class="h-12 w-12 text-[#fe5000]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No posters found</h3>
                    <p class="text-gray-500 mb-6 max-w-md mx-auto">
                        @if(request()->hasAny(['search', 'status']))
                            No posters match your current filters. Try adjusting your search criteria.
                        @else
                            Get started by creating your first poster to promote campaigns and events.
                        @endif
                    </p>
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700 leading-5">
                                Showing
                                <span class="font-medium">{{ $posters->firstItem() ?? 0 }}</span>
                                to
                                <span class="font-medium">{{ $posters->lastItem() ?? 0 }}</span>
                                of
                                <span class="font-medium">{{ $posters->total() }}</span>
                                results
                            </p>
                        </div>
                        ...
                        <div>
                            <span class="relative z-0 inline-flex shadow-sm rounded-md">
                                ...
                                @else
                                    <a href="{{ $posters->previousPageUrl() }}" rel="prev" class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-l-md leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150" aria-label="Previous">
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                        </svg>
                                    </a>
                                ...
                                @foreach ($posters->getUrlRange(1, $posters->lastPage()) as $page => $url)
                                    @if ($page == $posters->currentPage())
                                        <span aria-current="page">
                                            <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-[#fe5000] border border-[#fe5000] cursor-default leading-5">{{ $page }}</span>
                                        </span>
                                    @else
